...
public class GeometryWindow {
	...
	private CardLayout inputCards = new CardLayout(0, 0); //A CardLayout to switch between the different input frames. One for triangles for 3 inputs, one for rectangles and ellipses for 2 inputs, one blank
	private CardLayout outputCards= new CardLayout(0, 0); //Creating a CardLayout to switch between the different output frames. One for each shape, one blank
	private CardLayout shapeAInputCards = new CardLayout(0, 0); //A CardLayout to hold input frames for each shape ShapeA can be, rectangle or ellipse, triangle, or blank
	private CardLayout shapeBInputCards = new CardLayout(0, 0); //A CardLayout to hold input frames for each shape ShapeB can be, rectangle or ellipse, triangle, or blank
	public ShapeFactory shapeCreator = new ShapeFactory(); //ShapeFactory object that will be used to create a new shape
	private int calcMode, shapeAMode, shapeBMode = -1; //Variable to keep track of what kind of shape is being made
	private DecimalFormat doubleFormat = new DecimalFormat("###,####.##"); //Defining a format for the decimals to follow so they don't get too long.
	private NumberFormatter doubleFormatter = new NumberFormatter(doubleFormat); //A NumberFormatter to use as a mask so only digits get taken as input
	
	private double calcCircum, calcArea, shapeACircum, shapeAArea, shapeBCircum, shapeBArea; //Variables to hold the circumference and area of each shape we make
	
	private Shape shapeA = null; //Defining shapeA as null so we dont have to keep making a new shape
	private Shape shapeB = null; //Defining shapeA as null so we dont have to keep making a new shape
	...
	private void initialize() {
		...
		String[] shapes = {"","Ellipse", "Rectangle","Triangle"}; //A array of Strings holding options for the dropdown menus: shapes or nothing
		...
		JLabel shapePicture = new JLabel(""); //A Label to show a picture of the shape being made
		shapePicture.setBounds(250, 44, 75, 75);
		calculatePanel.add(shapePicture);
		
		JPanel outputPanel = new JPanel(); //Panel that'll hold the outputs to swtich between dependent on the shape being made
		...
		calculatePanel.add(outputPanel); //Add the panel to the caclulate tab
		outputPanel.setLayout(outputCards); //Set the layout of the panel to the cardlayout already made, but not filled yet
		
		JPanel blankOutput = new JPanel();
		blankOutput.setBackground(Color.DARK_GRAY);
		outputPanel.add(blankOutput, "name_1121216275611900");
		outputCards.addLayoutComponent(blankOutput,"blankOutput"); //Add blank Output to the stack of outputCards with its same title to search for
		...
		outputCards.addLayoutComponent(ellipseOutput,"ellipseOutput"); //Add ellipse Output to the stack of outputCards with its same title to search for
		
		/*Defining the labels that will be in the ellipse's output Panel*/
		...
		JLabel ellipCircumLabel = new JLabel("0"); //Label that shows the circumference of the ellipse that will be changed as we're making shapes
		...
		JLabel ellipAreaLabel = new JLabel("New label"); //Label that shows the area of the ellipse that will be changed as we're making shapes
		...
		JLabel isMajLargerLabel = new JLabel("is Major Axis Larger?");
		isMajLargerLabel.setForeground(Color.WHITE);
		isMajLargerLabel.setFont(new Font("Tahoma", Font.BOLD, 12));
		isMajLargerLabel.setBounds(32, 142, 134, 20);
		ellipseOutput.add(isMajLargerLabel);
		
		JLabel isCircleResult = new JLabel("X"); //Label to hold the result of isCircle() for ellipses
		...
		JLabel isMajorAxisLargerLabel = new JLabel("X"); //Label to hold the result of isMajorAxisLarger() for ellipses
		...
		JLabel isMinorLargerLabel = new JLabel("is Minor Axis Larger?");
		isMinorLargerLabel.setForeground(Color.WHITE);
		isMinorLargerLabel.setFont(new Font("Tahoma", Font.BOLD, 12));
		isMinorLargerLabel.setBounds(264, 142, 134, 20);
		ellipseOutput.add(isMinorLargerLabel);
		
		JLabel isMinorAxisLargerLabel = new JLabel("X"); //Label to hold the result of isMinorAxisLarger() for ellipses
		...
		/*Defining the labels that will be in the rectangle output*/
		...
		/*Defining the labels that will be in rectangle's output*/
		...
		JPanel twoInput = new JPanel(); //Panel for shapes that need only two inputs
		...
		doubleFormatter.setAllowsInvalid(false); //Forbids nondigits from being entered
		...
		comboBox.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				...
				if(selected.equals("Ellipse")) {
					...
					input1Field.setText(null); //Clear the input fields
					input2Field.setText(null); //Clear the input fields
					
					input1Label.setText("Major Axis:"); //Rename the input labels and move them to look nice
					input1Label.setBounds(30, 13, 86, 22);
					input2Label.setText("Minor Axis:");
					input2Label.setBounds(262, 13, 86, 22);
					input1Field.setBounds(146, 13, 86, 22);
					input2Field.setBounds(378, 13, 86, 22);					
					inputCards.show(inputPanel, "twoInput"); //Show the input needed
					outputCards.show(outputPanel,"blankOutput"); //Show the correct output
				}
		...
		comboBox.setModel(new DefaultComboBoxModel<String>(shapes));
			...
			public void actionPerformed(ActionEvent e) {
				if(calcMode == 1) {
					...
					try {
						...
						calcCircum = Math.round((ellip.getCircumference()*100.0)/100.0);
						calcArea = Math.round((ellip.getArea()*100.0)/100.0);
						ellipAreaLabel.setText(String.valueOf(calcArea));
						ellipCircumLabel.setText(String.valueOf(calcCircum));
					...
					try {
						...
						calcCircum = Math.round((rect.getCircumference()*100.0)/100.0);
						calcArea = Math.round((rect.getArea()*100.0)/100.0);
						rectAreaLabel.setText(String.valueOf(calcArea));
						rectCircumLabel.setText(String.valueOf(calcCircum));
					...
					try {
						...
						calcCircum = Math.round((tri.getCircumference()*100.0)/100.0);
						calcArea = Math.round((tri.getArea()*100.0)/100.0);
						triArea.setText(String.valueOf(calcArea));
						triCircum.setText(String.valueOf(calcCircum));
		...
		JComboBox<String> shapeACombo = new JComboBox<String>();
		...
		shapeACombo.setModel(new DefaultComboBoxModel<String>(shapes));
		...
		JLabel shapeBLabel = new JLabel("Shape B:");
		shapeBLabel.setForeground(Color.WHITE);
		shapeBLabel.setFont(new Font("Tahoma", Font.PLAIN, 12));
		shapeBLabel.setBounds(27, 11, 56, 22);
		shapeBPanel.add(shapeBLabel);
		...
		JLabel shapeAsideBLabel = new JLabel("Side b:");
		shapeAsideBLabel.setForeground(Color.WHITE);
		shapeAsideBLabel.setFont(new Font("Tahoma", Font.BOLD, 12));
		shapeAsideBLabel.setBounds(34, 42, 51, 22);
		bTriangleInput.add(shapeAsideBLabel);
		
		JLabel shapeBsideCLabel = new JLabel("Side c:");
		shapeBsideCLabel.setForeground(Color.WHITE);
		shapeBsideCLabel.setFont(new Font("Tahoma", Font.BOLD, 12));
		shapeBsideCLabel.setBounds(34, 73, 51, 22);
		bTriangleInput.add(sideCLabel);
		...
		JComboBox<String> shapeBCombo = new JComboBox<String>();
		...
		shapeBCombo.setModel(new DefaultComboBoxModel<String>(shapes));
				...
				if(shapeAMode < 3  && shapeAMode != -1) {
					...
					if(shapeAMode == 1) {
						try {
							...
							shapeACircum = Math.round((shapeA.getCircumference()*100.0)/100.0);
							shapeAArea = Math.round((shapeA.getArea()*100.0)/100.0);
							shapeAAreaLabel.setText(String.valueOf(shapeAArea));
							shapeACircumLabel.setText(String.valueOf(shapeACircum));
						}
						catch(IllegalArgumentException iae) {
				...
				if(shapeBMode < 3  && shapeBMode != -1) {
					...
					if(shapeBMode == 1) {
						...
						try {
							...
							shapeBCircum = Math.round((shapeB.getCircumference()*100.0)/100.0);
							shapeBArea = Math.round((shapeB.getArea()*100.0)/100.0);
							shapeBAreaLabel.setText(String.valueOf(shapeBArea));
							shapeBCircumLabel.setText(String.valueOf(shapeBCircum));
						}
						catch(IllegalArgumentException iae) {
							System.err.println("Invalid axis!");
						}
